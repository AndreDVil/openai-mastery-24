"""
Quick manual test for the JsonModeChatClient (p03b JSON Contract Plugin).
...
from .src.contracts.structured_answer_lite import StructuredAnswerLiteContract
...
def main() -> None:
    ...
    # 2) Choose a contract
    contract = StructuredAnswerLiteContract()

    # 3) Initialize the JSON Mode chat client (contract-driven)
    chat = JsonModeChatClient(client=client, contract=contract, debug=True, max_retries=2)
